...
/**
 * A {@link com.google.common.hash.PrimitiveSink} that can compute a hash code after reading the input. Each hasher should
 ...
 * {@link #putString(CharSequence, java.nio.charset.Charset)} is generally only useful for cross-language
 * compatibility (otherwise prefer {@link #putUnencodedChars}). However, the character encodings
 * must be identical across languages. Also beware that {@link java.nio.charset.Charset} definitions may occasionally
 * change between Java releases.
 *
 * <p><b>Warning:</b> Chunks of data that are put into the {@link com.google.common.hash.Hasher} are not delimited.
 * The resulting {@link com.google.common.hash.HashCode} is dependent only on the bytes inserted, and the order in which
 ...
 * {@link #putString(CharSequence, java.nio.charset.Charset)} should only be used consistently with <i>complete</i>
