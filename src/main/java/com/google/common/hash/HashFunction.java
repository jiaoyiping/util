 ...
 *     multibyte values (via {@link com.google.common.hash.Hasher}), but this is merely a convenience; these are
 ...
 *     length (given by {@link #bits}). For example, {@link com.google.common.hash.Hashing#sha1} produces a
 *     160-bit number, while {@link com.google.common.hash.Hashing#murmur3_32()} yields only 32 bits. Because a
 *     {@code long} value is clearly insufficient to hold all hash code values, this API
 *     represents a hash code as an instance of {@link com.google.common.hash.HashCode}.
 *
 * <li><b>pure function:</b> the value produced must depend only on the input bytes, in
 *     the order they appear. Input data is never modified. {@link com.google.common.hash.HashFunction} instances
 ...
 *     used.
 *
 * <li><b>cryptographic:</b> certain hash functions such as {@link com.google.common.hash.Hashing#sha512} are
 ...
 * {@link com.google.common.hash.Hasher}. Obtain a new hasher from the hash function using {@link #newHasher},
 * "push" the relevant data into it using methods like {@link com.google.common.hash.Hasher#putBytes(byte[])},
 * and finally ask for the {@code HashCode} when finished using {@link com.google.common.hash.Hasher#hash}. (See
 * an {@linkplain #newHasher example} of this.)
 *
 * <p>If all you want to hash is a single byte array, string or {@code long} value, there
 * are convenient shortcut methods defined directly on {@link com.google.common.hash.HashFunction} to make this
 * easier.
 *
 * <p>Hasher accepts primitive data types, but can also accept any Object of type {@code
 * T} provided that you implement a {@link com.google.common.hash.Funnel Funnel<T>} to specify how to "feed" data
 * from that object into the function. (See {@linkplain com.google.common.hash.Hasher#putObject an example} of
 * this.)
 *
