...
/**
 * A manager for monitoring and controlling a set of {@linkplain com.google.common.util.concurrent.Service services}. This class
 * provides methods for {@linkplain #startAsync() starting}, {@linkplain #stopAsync() stopping} and
 * {@linkplain #servicesByState inspecting} a collection of {@linkplain com.google.common.util.concurrent.Service services}.
 * Additionally, users can monitor state transitions with the {@linkplain com.google.common.util.concurrent.ServiceManager.Listener listener}
 * mechanism.
 *
...
public final class ServiceManager {
   ...
   * com.google.common.util.concurrent.Service service} starts, or terminates), should attach {@linkplain com.google.common.util.concurrent.Service.Listener service
  ...
  public abstract static class Listener {
     ...
     * {@linkplain com.google.common.util.concurrent.Service.State#RUNNING running} state. If any services fail during start up or
     * {@linkplain com.google.common.util.concurrent.Service.State#FAILED fail}/{@linkplain com.google.common.util.concurrent.Service.State#TERMINATED terminate} before all other
     * services have started {@linkplain com.google.common.util.concurrent.Service.State#RUNNING running} then this method will not be called.
     */
    public void healthy() {}

    /**
     * Called when the all of the component services have reached a terminal state, either
     * {@linkplain com.google.common.util.concurrent.Service.State#TERMINATED terminated} or {@linkplain com.google.common.util.concurrent.Service.State#FAILED failed}.
     */
    public void stopped() {}

    /**
     * Called when a component service has {@linkplain com.google.common.util.concurrent.Service.State#FAILED failed}.
     *
  ...
  /**
   * An encapsulation of all of the state that is accessed by the {@linkplain com.google.common.util.concurrent.ServiceManager.ServiceListener
   * service listeners}.  This is extracted into its own object so that {@link com.google.common.util.concurrent.ServiceManager.ServiceListener}
   * could be made {@code static} and its instances can be safely constructed and added in the
   * {@link com.google.common.util.concurrent.ServiceManager} constructor without having to close over the partially constructed
   * {@link com.google.common.util.concurrent.ServiceManager} instance (i.e. avoid leaking a pointer to {@code this}).
   */
   ...
   *
   * @throws IllegalArgumentException if not all services are {@linkplain com.google.common.util.concurrent.Service.State#NEW new} or if there
  ...
  /**
   * Registers a {@link com.google.common.util.concurrent.ServiceManager.Listener} to be {@linkplain java.util.concurrent.Executor#execute executed} on the given
   ...
   * {@linkplain com.google.common.util.concurrent.Service#startAsync started}.
   *
   ...
   * calling {@link #addListener(com.google.common.util.concurrent.ServiceManager.Listener)}.
   *
  ...
  /**
   * Registers a {@link com.google.common.util.concurrent.ServiceManager.Listener} to be run when this {@link com.google.common.util.concurrent.ServiceManager} changes state. The
   * listener will not have previous state changes replayed, so it is suggested that listeners are
   * added before any of the managed services are {@linkplain com.google.common.util.concurrent.Service#startAsync started}.
   *
   * <p>{@code addListener} guarantees execution ordering across calls to a given listener but not
   * across calls to multiple listeners. Specifically, a given listener will have its callbacks
   * invoked in the same order as the underlying service enters those states. Additionally, at most
  ...
  /**
   * Initiates service {@linkplain com.google.common.util.concurrent.Service#startAsync startup} on all the services being managed.
   * It is only valid to call this method if all of the services are {@linkplain com.google.common.util.concurrent.Service.State#NEW new}.
   *
   * @return this
   * @throws IllegalStateException if any of the Services are not {@link com.google.common.util.concurrent.Service.State#NEW new} when the
   *     method is called.
   */
  ...
  /**
   * Waits for the {@link com.google.common.util.concurrent.ServiceManager} to become {@linkplain #isHealthy() healthy}.  The manager
   * will become healthy after all the component services have reached the {@linkplain com.google.common.util.concurrent.Service.State#RUNNING
   * running} state.
   *
  ...
  /**
   * Waits for the {@link com.google.common.util.concurrent.ServiceManager} to become {@linkplain #isHealthy() healthy} for no more
   * than the given time.  The manager will become healthy after all the component services have
   * reached the {@linkplain com.google.common.util.concurrent.Service.State#RUNNING running} state.
   *
   * @param timeout the maximum time to wait
   * @param unit the time unit of the timeout argument
   * @throws java.util.concurrent.TimeoutException if not all of the services have finished starting within the deadline
   * @throws IllegalStateException if the service manager reaches a state from which it cannot
   *     become {@linkplain #isHealthy() healthy}.
   */
  ...
  /**
   * Initiates service {@linkplain com.google.common.util.concurrent.Service#stopAsync shutdown} if necessary on all the services
   * being managed.
   *
   * @return this
   */
   ...
   * services will either be {@linkplain com.google.common.util.concurrent.Service.State#TERMINATED terminated} or {@linkplain
   ...
   * this method returns all services will either be {@linkplain com.google.common.util.concurrent.Service.State#TERMINATED
   * terminated} or {@linkplain com.google.common.util.concurrent.Service.State#FAILED failed}.
   *
   * @param timeout the maximum time to wait
   * @param unit the time unit of the timeout argument
   * @throws java.util.concurrent.TimeoutException if not all of the services have stopped within the deadline
   */
  ...
  /**
   * Returns true if all services are currently in the {@linkplain com.google.common.util.concurrent.Service.State#RUNNING running} state.
   *
  ...
  /**
   * An encapsulation of all the mutable state of the {@link com.google.common.util.concurrent.ServiceManager} that needs to be
   * accessed by instances of {@link com.google.common.util.concurrent.ServiceManager.ServiceListener}.
   */
  private static final class ServiceManagerState {
     ...
     * {@link com.google.common.util.concurrent.Service#startAsync()}.
     */
    ...
    /**
     * Marks the {@link com.google.common.util.concurrent.Service.State} as ready to receive transitions. Returns true if no transitions have
     * been observed yet.
     */
    void markReady() {
      monitor.enter();
  ...
  /**
   * A {@link com.google.common.util.concurrent.Service} that wraps another service and times how long it takes for it to start and
   * also calls the {@link com.google.common.util.concurrent.ServiceManager.ServiceManagerState#transitionService(com.google.common.util.concurrent.Service, com.google.common.util.concurrent.Service.State, com.google.common.util.concurrent.Service.State)},
  ...
  /**
   * A {@link com.google.common.util.concurrent.Service} instance that does nothing.  This is only useful as a placeholder to
   * ensure that the {@link com.google.common.util.concurrent.ServiceManager} functions properly even when it is managing no services.
   *
